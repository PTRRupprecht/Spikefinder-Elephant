...
from elephant.utils2 import map_between_spaces
...
for training in range(0,2):
    ...
    for iii, n_dataset in enumerate(range(1,nb_datasets+1)):
        if training:
            print('Processing training dataset %s' % n_dataset)
            ...
            print('Processing test dataset %s' % n_dataset)
        ...
        if training:
            AA = sio.loadmat('statEmbedding/embedding_spacesX.mat')['embedding_2D_stats']
            distances = map_between_spaces(AA[iii,:])
        else:
            AA = sio.loadmat('statEmbedding/embedding_spacesX.mat')['embedding_2D_stats33']
            distances = map_between_spaces(AA[iii,:])
